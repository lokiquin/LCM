...
//
{
    ...
    a2tree->ReadFile("DISK_NGC7814.txt", "kt:lum2");
    cout << "NGC7814 Reported tree has " << a2tree->GetEntries() << " entries" << endl;
    
    // ***** Set alpha filename here ***** //
    a5tree->ReadFile("n7814_LCM_DiskXS.txt", "kt:lum5");
    cout << "anatA tree has " << a5tree->GetEntries() << " entries" << endl;
    
    // ***** Set alpha filename here ***** //
    a1tree->ReadFile("n7814_LCM_DiskAB.txt", "kt:lum1");
    ...
    Double_t xMin=0.0, xMax=20.;
    Double_t yMin=0.0, yMax=240.;
    ...
    leg->SetFillColor(0);
    leg->AddEntry(a2graph, "Reported Disk Profile Fraternalli ", "l");
    leg->AddEntry(a5graph, "LCM Disk Profile for MW XueSofue", "l"); 
    leg->AddEntry(a1graph, "LCM Disk Profile for MW Klypin (2002) model B   ", "l");
