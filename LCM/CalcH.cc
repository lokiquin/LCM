/*
 ...
 Created by:NOAH OBLATH, Richard Ott, David Chester
 Date:6/19/2012
 */
...
// 
//STANDARD OUTPUT: radius,  vlum 
//cout <<other->r(other_i) << "   "<<other->vlum(other_i)<< endl;
//KAPPA <<"   "<<(1.-1./other->n(other_i)) << "   "<<(1.-1./milky->n(mw_i)) << endl;
//  analysis chunks:r  num:v2	part(a)denom: v2  part(b)denom: v2 
//			 radius 
//cout<<other->r(other_i)<< "   "<<((sqrt((1.+other->vlum(other_i)*overc)/(1.-other->vlum(other_i)*overc)))+(other->n(other_i)/milky->n(mw_i)))<< "   "<<(sqrt((1.+other->vlum(other_i)*overc)/(1.-other->vlum(other_i)*overc)))<< "   "<<(other->n(other_i)/milky->n(mw_i)) << "  "  <<(sqrt((1.+other->vlum(other_i)*overc)/(1.-other->vlum(other_i)*overc)))<<"   "<<(1.*other->n(other_i)/milky->n(mw_i))<< endl;

//meagan edit: just radius, denom part (a), and denom part(b) of v2, potential gal, potential milky,
//cout<<other->r(other_i)<< "  "  <<(sqrt((1.+other->vlum(other_i)*overc)/(1.-other->vlum(other_i)*overc)))<<"  "<<(1-1.*other->n(other_i)/milky->n(mw_i))<< "   "<<other->vlum(other_i)<< endl;
//
//
//
//<< "   "<<other->vlum(other_i)<< "   " <<((2/(other->n(other_i)/milky->n(mw_i)+1/other->n(other_i)/milky->n(mw_i)))-1)<< endl;
//cout <<other->r(other_i) <<  "  "  <<((1.-1./other->n(other_i))/(1.-1./milky->n(mw_i)))<< "   " <<(((sqrt((1.+other->vlum(other_i)*overc)/(1.-other->vlum(other_i)*overc)))+(other->n(other_i)/milky->n(mw_i)))/((sqrt((1.+other->vlum(other_i)*overc)/(1.-other->vlum(other_i)*overc)))-(other->n(other_i)/milky->n(mw_i)))) << "   "<<((2/(other->n(other_i)/milky->n(mw_i)+1/other->n(other_i)/milky->n(mw_i)))-1) << "   "<<other->vlum(other_i)<< endl;
	//1st column radius, 2nd Kappa,  v2 3rd column v2, 4th column v1, 5th vlum
     //(((other->vlum_f()/c)+((other->vlum_f()/c)*(other->vlum_f()/c))-(0.5)*(other->phi_f()-milky>phi_f()))/(2.0+(other->vlum_f()/c)+((other->vlum_f()/c)*(other->vlum_f()/c))+(0.5)*(other->phi_f()-milky->phi_f())));
//cout <<other->r(other_i)<< "   "<<other->vlum(other_i)<< endl;
	//cout <<other->r(other_i) <<  "  "  <<milky->vlum(mw_i) <<endl;
  //((1.-1./other->n(other_i))/(1.-1./milky->n(mw_i)))<<  "  "  << kappa(i)<< "  "  
//2nd column normalization
//kappa: all valid
	//cout <<(1.-1./other->n(other_i))/(1.-1./milky->n(mw_i))<< "  "  <<kappa_f()<< "    " <<(other->phi(other_i)/milky->phi(mw_i)) <<endl;
cout <<other->r(other_i) <<  "  "<< sqrt(1-milky->phi(mw_i))/sqrt(1-other->phi(other_i))  <<endl;
	}
    //Print(&CalcH::vLCM);
    //Print(&Galaxy::vlum);
